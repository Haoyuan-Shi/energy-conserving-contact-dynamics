...
class PairBodyRoundedPolygon : public Pair {
 ...
 protected:
  ...
  int vertex_against_edge(int i, int j, double k_n, double k_na, double **x, double** v, double** angmom, double **f,
                          double **torque, tagint *tag, double &evdwl, double *facc);
  ...
  int compute_distance_to_vertex(int ibody, int edge_index, double *xmi, double rounded_radius,
                                 ...
                                 double hi[3], double &t, int &contact, double &anglesum);
  // compute contact forces if contact points are detected
  void contact_forces(double *xpi, int ibody, int ni, double *xpj, int jbody, int nj, double fx, double fy, double fz,
