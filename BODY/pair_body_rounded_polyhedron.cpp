...
#define MAX_CONTACTS 256  // for 3D models (including duplicated counts)

//#define _POLYHEDRON_DEBUG

enum {EE_INVALID=0,EE_NONE,EE_INTERACT,EV_INTERACT};
enum {VF_INVALID=0,VF_NONE,VF_OUTSIDE,VF_INSIDE,VF_INTERSECT_VE};
...
void PairBodyRoundedPolyhedron::compute(int eflag, int vflag)
{
  ...
  double rsq,eradi,eradj,rradi,rradj;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      double r = sqrt(rsq);
      if (r > eradi + eradj + rradi + rradj + cut_inner) continue;
      ...
      #endif
      vertex_against_face(i, j, itype, jtype, x, evdwl, facc);

      // check interaction between j's edges and i' faces
      #ifdef _POLYHEDRON_DEBUG
      printf("\nINTERACTION between edges of %d vs. faces of %d:\n", j, i);
      #endif
      vertex_against_face(j, i, jtype, itype, x, evdwl, facc);
      ...
      edge_against_edge(i, j, itype, jtype, x, evdwl, facc);
...
{
  int ni,nfi,inside,ifirst,iffirst,npi1,npi2,npi3,ibonus,tmp,SF=0;
  ...
  for (ni = 0; ni < nfi; ni++) {
    ...
    // Declare outputs
    double e1_1[3], e1_2[3], r_min=DELTA;
    double e2_1[3], e2_2[3], r_min2=DELTA;

    inside_polygon_d(ibody, ni, x[ibody], h, nullptr, inside, tmp, e1_1, e1_2, r_min, e2_1, e2_2, r_min2);


    if (d <= contact_dist + cut_inner) {
      if (inside) {
...
if (SF == 0){
  double emin_1[3], emin_2[3], rmin_min = DELTA;
  int DD = 0;
  for (ni = 0; ni < nfi; ni++) {
      ...
      if (d <= contact_dist + cut_inner) {
        if (!inside && r_min < contact_dist + cut_inner) {
          double h[3], r12, t;
          project_pt_line(x[jbody], e1_1, e1_2, h, r12, t);
          if (r12 < rmin_min){
            rmin_min = r12;
            emin_1[0] = e1_1[0];
            emin_1[1] = e1_1[1];
    ...
    if (DD == 1){
      double h[3], r12, t;
      project_pt_line(x[jbody], emin_1, emin_2, h, r12, t);
...
int PairBodyRoundedPolyhedron::edge_against_edge(int ibody, int jbody,
  int itype, int jtype, double** x, double &evdwl, double* facc)
{
...
int PairBodyRoundedPolyhedron::vertex_against_face(int ibody, int jbody,
  int itype, int jtype, double** x, double &evdwl, double* facc)
{
  int ni,nei,nj,nfj,interact,interact_round;
  double rradi,rradj,energy;
  ...
  for (ni = 0; ni < nei; ni++) {
    ...
    double e_min_1[3], e_min_2[3], r12_min=DELTA;
    ...
    for (nj = 0; nj < nfj; nj++) {
      ...
      double e1_1[3], e1_2[3], r12=DELTA;

      interact = interaction_face_to_vertex(jbody, nj, x[jbody], rradj,
                                          ibody, ni, x[ibody], rradi,
                                          itype, jtype, cut_inner,
                                          e1_1, e1_2, r12,
                                          energy, facc);
       
      if (interact == VF_OUTSIDE) {
        skip_outer = 0;
    ...
    if (skip_outer == 1) {
      interact = interaction_edge_to_vertex(jbody, rradj,
                                            ibody, ni, x[ibody], rradi,
                                            itype, jtype, cut_inner,
                                            e_min_1, e_min_2, r12_min,
                                            energy, facc);
    }
  ...
  int itype, int jtype, double cut_inner, double &energy, double* facc)
{
  ...
  double edge_length_1 = sqrt((xpj1[0]-xpj2[0])*(xpj1[0]-xpj2[0])+(xpj1[1]-xpj2[1])*(xpj1[1]-xpj2[1])+(xpj1[2]-xpj2[2])*(xpj1[2]-xpj2[2]));
  double edge_length_2 = sqrt((xi1[0]-xi2[0])*(xi1[0]-xi2[0])+(xi1[1]-xi2[1])*(xi1[1]-xi2[1])+(xi1[2]-xi2[2])*(xi1[2]-xi2[2]));
  double t1_round = contact_dist / edge_length_1;
  double t2_round = contact_dist / edge_length_2;
  ...
  if (t1 >= 0 && t1 <= 1 && t2 >= 0 && t2 <= 1 &&
  ...
  } else if (r < contact_dist + cut_inner) {
      double r12_vals[4];
      double h[3], r12, t;

      // 1. xi1 to edge xpj1-xpj2
      project_pt_line(xi1, xpj1, xpj2, h, r12, t);
      r12_vals[0] = r12;

      // 2. xi2 to edge xpj1-xpj2
      project_pt_line(xi2, xpj1, xpj2, h, r12, t);
      r12_vals[1] = r12;

      // 3. xpj1 to edge xi1-xi2
      project_pt_line(xpj1, xi1, xi2, h, r12, t);
      r12_vals[2] = r12;

      // 4. xpj2 to edge xi1-xi2
      project_pt_line(xpj2, xi1, xi2, h, r12, t);
      r12_vals[3] = r12;

      // Find minimum
      double r12_min = r12_vals[0];
      int min_index = 0;
      for (int ii = 1; ii < 4; ++ii) {
          if (r12_vals[ii] < r12_min) {
              r12_min = r12_vals[ii];
              min_index = ii;
          }
      }
    ...
    double closest_pt1[3], closest_pt2[3];  // To store the final two points
    ...
    if (min_index == 0) {
        ...
        for (int i = 0; i < 3; ++i) {
            closest_pt1[i] = h[i];
            closest_pt2[i] = xpj1[i];
        }
    } else if (min_index == 3) {
        project_pt_line(xpj2, xi1, xi2, h, r12, t);
        for (int i = 0; i < 3; ++i) {
            closest_pt1[i] = h[i];
            closest_pt2[i] = xpj2[i];
        }
    }

    if (r12_min < contact_dist + cut_inner) {
      pair_force_and_torque(jbody, ibody, closest_pt2, closest_pt1, r12_min, contact_dist,
                      jtype, itype, x, v, f, torque, angmom,
                      jflag, energy, facc);
      interact = EV_INTERACT;   
    }
  } 
  return interact;
}
...
/* -------------------------------------------------------------------------
  ...
  Input:
    ...
    rounded_radius_i = rounded radius of the body i
    jbody      = body i (i.e. atom j)
    vertex_index = coordinate of the tested vertex from another body
    xmj        = atom j's coordinates (body j's center of mass)
    rounded_radius_j = rounded radius of the body j
    cut_inner  = cutoff for vertex-vertex and vertex-edge interaction
  Output:
...
int PairBodyRoundedPolyhedron::interaction_face_to_vertex(int ibody,
  int face_index, double *xmi, double rounded_radius_i,
  int jbody, int vertex_index, double *xmj, double rounded_radius_j,
  int itype, int jtype, double cut_inner, double *e1_1, double *e1_2, double &r12, double &energy, double* facc)
{
  if (face_index >= facnum[ibody]) return VF_INVALID;

  int ifirst,iffirst,jfirst,npi1,npi2,npi3,tmp;
  ...
  xpj1[0] = xmj[0] + discrete[jfirst+vertex_index][0];
  xpj1[1] = xmj[1] + discrete[jfirst+vertex_index][1];
  xpj1[2] = xmj[2] + discrete[jfirst+vertex_index][2];
  ...
  double hi1[3], d1, contact_dist;
  int inside1 = 0;

  project_pt_plane(xpj1, xi1, xi2, xi3, hi1, d1, inside1);

  int interact = VF_INSIDE;

  // Declare outputs
  double r_min=DELTA;
  double e2_1[3], e2_2[3], r_min2=DELTA;

  inside_polygon_d(ibody, face_index, xmi, hi1, nullptr, inside1, tmp, e1_1, e1_2, r_min, e2_1, e2_2, r_min2);
  ...
  if (d1 <= contact_dist + cut_inner) {
    if (inside1) {
      ...
      printf(" - compute pair force between vertex %d from edge %d of body %d "
        ...
        npj1, vertex_index, jbody, face_index, ibody, d1);
      #endif

      return interact = VF_OUTSIDE;

    }

    if (r_min < contact_dist + cut_inner && r_min > 0) {
      double h[3], t;
      project_pt_line(xpj1, e1_1, e1_2, h, r12, t);
    } 
  }
...
/* -------------------------------------------------------------------------
  Compute the interaction between a face of body i and an edge from
  another body (with the case that a point of an edge is outside a surface 
  but within the contact_dist)
  Input:
    ibody      = body i (i.e. atom i)
    face_index = face index of body i
    xmi        = atom i's coordinates (body i's center of mass)
...
int PairBodyRoundedPolyhedron::interaction_edge_to_vertex(int ibody, double rounded_radius_i,
  int jbody, int vertex_index, double *xmj, double rounded_radius_j,
  ...
  double &energy, double* facc)
{
  ...
  if (r12 <= contact_dist + cut_inner) {
    double h[3], t;
    project_pt_line(xpj1, e1_1, e1_2, h, r12, t);
    pair_force_and_torque(jbody, ibody, xpj1, h, r12, contact_dist,
                          jtype, itype, x, v, f, torque, angmom,
                          jflag, energy, facc);        
  }

  int interact = VF_INTERSECT_VE;
...

{
  ...
  for (i = 0; i < MAX_FACE_SIZE; i++) {
    ...
    // printf("magu = %f, magv = %f, magu * magv = %f, EPSILON*rradsq=%f, inside1=%d\n",magu,magv,magu * magv, EPSILON*rradsq, inside1);
...
/* ----------------------------------------------------------------------
  Check if points q1 and q2 are inside a convex polygon, i.e. a face of
  a polyhedron and return the shortest distance and edge
    ibody       = atom i's index
    face_index  = face index of the body
    xmi         = atom i's coordinates
    q1          = tested point on the face (e.g. the projection of a point)
...
void PairBodyRoundedPolyhedron::inside_polygon_d(int ibody, int face_index,
                            double* xmi, const double* q1, const double* q2,
                            int& inside1, int& inside2, 
                            double* e_out1_1, double* e_out1_2, double& r_out1,
...

{
  ...
  for (i = 0; i < MAX_FACE_SIZE; i++) {
    ...
    double h1[3], r1, t1;
    project_pt_line(q1, xi1, xi2, h1, r1, t1);

    if (r1 < r_out1) {
      r_out1 = r1;
      MathExtra::copy3(xi1, e_out1_1);
      MathExtra::copy3(xi2, e_out1_2);
    }

    if (q2 != nullptr) {
      double h2[3], r2, t2;
      project_pt_line(q2, xi1, xi2, h2, r2, t2);

      if (r2 < r_out2) {
        r_out2 = r2;
        MathExtra::copy3(xi1, e_out2_1);
        MathExtra::copy3(xi2, e_out2_2);
      }
    }
  ...
  if (inside1) r_out1 = -r_out1;
  if (q2 != nullptr) {
    if (inside2) r_out2 = -r_out2;
  }
}
     ...
     const double* xi1, const double* xi2, double* h, double& d, double& t)
{
  ...
  if (max_diff > EPSILON) {
      t = (h[max_dim] - xi1[max_dim]) / (xi2[max_dim] - xi1[max_dim]);
  } else {
      error->all(FLERR, "Incorrect project_pt_line: line segment is degenerate");
  }
                  ...
                  double* h1, double* h2, double& t1, double& t2, double& r)
{
  ...
  if (fabs(dot - 1.0) < EPSILON*EPSILON*EPSILON*EPSILON) {
    ...
    // Compute t13
    max_dim = 0;
    max_diff = fabs(x4[0] - x3[0]);
    for (int d = 1; d < 3; ++d) {
        double diff = fabs(x4[d] - x3[d]);
        if (diff > max_diff) {
            max_diff = diff;
            max_dim = d;
        }
    ...
    if (max_diff > EPSILON)
        t13 = (x13[max_dim] - x3[max_dim]) / (x4[max_dim] - x3[max_dim]);
    else
        error->all(FLERR, "Degenerate segment for t13");
    ...
    if (max_diff > EPSILON)
        t23 = (x23[max_dim] - x3[max_dim]) / (x4[max_dim] - x3[max_dim]);
    else
        error->all(FLERR, "Degenerate segment for t23");

    // Compute t31
    max_dim = 0;
    max_diff = fabs(x23[0] - x13[0]);
    for (int d = 1; d < 3; ++d) {
        double diff = fabs(x23[d] - x13[d]);
        if (diff > max_diff) {
            max_diff = diff;
            max_dim = d;
        }
    ...
    if (max_diff > EPSILON)
        t31 = (x3[max_dim] - x13[max_dim]) / (x23[max_dim] - x13[max_dim]);
    else
        error->all(FLERR, "Degenerate segment for t31");
    ...
    if (max_diff > EPSILON)
        t41 = (x4[max_dim] - x13[max_dim]) / (x23[max_dim] - x13[max_dim]);
    else
        error->all(FLERR, "Degenerate segment for t41");
    ...
    if (fabs(t31) < fabs(t41)) {
      t1 = t31;
    } else {
      t1 = t41;
    }

    if (fabs(t13) < fabs(t23)) {
      t2 = t13;
    } else {
      t2 = t23;
    }    
  ...
  int max_dim = 0;
  double max_diff = fabs(x2[0] - x1[0]);

  for (int d = 1; d < 3; ++d) {
      double diff = fabs(x2[d] - x1[d]);
      if (diff > max_diff) {
          max_diff = diff;
          max_dim = d;
      }
  ...
  if (max_diff > EPSILON)
      t1 = (h1[max_dim] - x1[max_dim]) / (x2[max_dim] - x1[max_dim]);
  else
      error->all(FLERR, "Degenerate segment for t1");
